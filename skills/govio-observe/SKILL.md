...
所有数据探查操作通过 `uvx --from skills/govio/assets/govio-*.whl govio-cli observe` 命令组执行。确保：

1. 已运行 `uvx --from skills/govio/assets/govio-*.whl govio-cli onboard` 完成初始化配置
...
| 步骤 | 子 Skill | CLI 命令 |
|---|---|---|
| 数据集操作 | `observe-dataset-ops` | `uvx --from skills/govio/assets/govio-*.whl govio-cli observe show-datasource / list / load / release` |
| 探查关系 | `observe-explore-relations` | `uvx --from skills/govio/assets/govio-*.whl govio-cli observe explore` |
| 比对数据 | `observe-compare-dfs` | `uvx --from skills/govio/assets/govio-*.whl govio-cli observe compare` |
...
> [执行 uvx --from skills/govio/assets/govio-*.whl govio-cli observe show-datasource]
...
> [执行 uvx --from skills/govio/assets/govio-*.whl govio-cli observe load] (多次)
> → 加载 customers, orders, addresses 等表

> [执行 uvx --from skills/govio/assets/govio-*.whl govio-cli observe explore]
> → 发现外键关系和相似列
...
**使用 Skill**: [子 Skill 名称]
**CLI 命令**: [对应的 uvx --from skills/govio/assets/govio-*.whl govio-cli 命令]
**输入**: [参数]
**预期输出**: [结果]
