...
description: 探查 DataFrame 之间的关系。当需要发现表之间的关联、分析数据血缘、或寻找潜在的外键关系时使用。通过 uvx --from skills/govio/assets/govio-*.whl govio-cli observe explore 命令执行，基于列名相似度和值重叠率推断关系。
---
...
```bash
uvx --from skills/govio/assets/govio-*.whl govio-cli observe explore
```
...
探查结果可通过 `uvx --from skills/govio/assets/govio-*.whl govio-cli observe visualize-relations` 生成图谱数据：
...
- 使用 `uvx --from skills/govio/assets/govio-*.whl govio-cli observe visualize-relations` 生成关系图谱
