...
```bash
govio-cli observe explore [--dataframes df1 df2 ...]
```
...
```bash
govio-cli observe load --name customers --datasource prod_db --sql "SELECT * FROM customers"
govio-cli observe load --name orders --datasource prod_db --sql "SELECT * FROM orders"
```
...
**执行**:
```bash
govio-cli observe explore --dataframes customers orders
```
