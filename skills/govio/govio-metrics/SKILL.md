...
针对常见场景的详细操作指南，按步骤给出具体命令和示例。

| Playbook | 适用场景 |
|----------|---------|
| [单指标 + 维度过滤取数](playbooks/simple-metric-query.md) | 用户问题包含**一个明确指标**和**一个维度筛选条件**（如"外滩业务中心的预计销售额"、"华东区本月账单收入"）。要求：指标为原子指标，或派生指标的依赖原子指标均可直接查到 |

> 以下场景**不适用**该 playbook，需按本文档步骤灵活处理：
> - 多指标组合对比（如"签约额和账单收入对比"）→ 按 Step 2 分别查询后组合
> - 环比/同比分析 → 使用 CTE 组合查询
> - 无维度筛选的全量汇总 → 跳过维度探查
...
- `report_ym`（报告年月）：数据按月更新的拉链字段，格式 `YYYYMM`（如 `202605`）
  ...
  - 用户未指定时间时，通过 Step 3.5 探查确认最新可用月份，**不要假设当前月份有数据**
