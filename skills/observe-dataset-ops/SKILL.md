...
数据集操作的统一入口，包含四个核心功能，对应 `govio-cli observe` 的子命令：

| 功能 | CLI 命令 | 用途 |
|---|---|---|
| 列出数据源 | `govio-cli observe show-datasource` | 查看可用数据库连接 |
| 列出 DataFrame | `govio-cli observe list` | 查看已加载的数据 |
| 加载数据 | `govio-cli observe load --name <name> --datasource <ds> --sql <sql>` | 从数据库抽取数据并持久化 |
| 在已加载的 DataFrame 上查询 | `govio-cli observe load --name <name> --memory --sql <sql>` | 不连数据库，在 store 内已有 DataFrame 上跑 SQL 二次加工 |
| 释放资源 | `govio-cli observe release --name <name>` | 删除 DataFrame 文件 |

> `load` 子命令的 `--datasource` 和 `--memory` 互斥：前者从数据库抽取，后者在已加载的 DataFrame 之上做查询/连接/聚合。详见下文「功能 3」。
>
> `load` 还支持 `-o / --output <file>`：把加载/查询得到的 DataFrame 内容以 JSON（`records` 方式、UTF-8、缩进 2）写出到磁盘文件，返回结果会额外带 `output_file` 字段。该参数对 `--datasource` 和 `--memory` 两种模式都适用。**使用 `-o` 输出数据集内容前，必须先征得用户确认**——如果用户未明确表示可以查看数据内容，应先问询是否允许输出。

## 前置条件

1. 已运行 `govio-cli onboard` 完成初始化
...
| --datasource | keyword | 二选一 | 数据源名称（从 show-datasource 获取），从数据库抽取 |
...
**执行**:
```bash
govio-cli observe load --name customers --datasource prod_db --sql "SELECT customer_id, name, email, created_at FROM customers"
```
...
{
  "success": true,
  "released": ["customers", "orders", "products"],
  "count": 3
}
```
...
2. **工作流结尾释放**: 每个 Plan 的最后一步使用 `release --all` 统一清理
3. **小批量加载**: 避免一次性加载大量数据
