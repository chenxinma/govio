...
| 加载数据 | `uvx --from skills/govio/assets/govio-*.whl govio-cli observe load --name <name> --datasource <ds> --sql <sql>` | 从数据库抽取数据并持久化 |
| 释放资源 | `uvx --from skills/govio/assets/govio-*.whl govio-cli observe release --name <name>` | 删除 DataFrame 文件 |
...
```bash
uvx --from skills/govio/assets/govio-*.whl govio-cli observe load --name customers --datasource prod_db --sql "SELECT customer_id, name, email FROM customers WHERE created_at > '2024-01-01'"
```
...
**执行**:
```bash
uvx --from skills/govio/assets/govio-*.whl govio-cli observe load --name customers --datasource prod_db --sql "SELECT customer_id, name, email, created_at FROM customers"
```
...
| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| --name | keyword | 是 | DataFrame 名称 |

### 调用方式

```bash
uvx --from skills/govio/assets/govio-*.whl govio-cli observe release --name customers
```
