...
| 加载数据 | `govio-cli observe load <name> <datasource> <sql>` | 从数据库抽取数据并持久化 |
| 释放资源 | `govio-cli observe release <name>` | 删除 DataFrame 文件 |
...
```bash
govio-cli observe load customers prod_db "SELECT customer_id, name, email FROM customers WHERE created_at > '2024-01-01'"
```
...
**执行**:
```bash
govio-cli observe load customers prod_db "SELECT customer_id, name, email, created_at FROM customers"
```
...
```bash
govio-cli observe release customers
```
