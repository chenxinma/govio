...
文章指出，在迭代开发 Agent 技能（如 Codex）时，很难凭感觉判断改进效果。Eval（评估）是将"感觉更好"转化为"可证明"的关键手段。

### 什么是 Eval

Eval = 提示词 → 捕获运行过程（trace + 产物）→ 一组检查 → 可比较的分数
...
- **结果目标**：任务是否完成？应用能否运行？
- **过程目标**：是否按预期调用了技能、使用了工具？
- **风格目标**：输出是否符合约定的规范？
- **效率目标**：是否避免了不必要的命令或 token 浪费？

**2. 创建 Skill**
Codex 的 skill 是一个包含 `SKILL.md` 的目录，其中 YAML front matter 的 `name` 和 `description` 决定了 skill 何时被触发。文章以一个"搭建 Vite + React + Tailwind 演示应用"的 skill 为例。

**3. 手动运行发现边界**
使用 `codex exec --full-auto` 运行，发现隐藏假设：

- 触发假设：什么情况下 skill 应该/不应该被调用
- 环境假设：是否假设空目录、npm 可用等
- 执行假设：是否会跳过某些步骤

**4. 构建小型提示集**
用 10-20 条提示的 CSV 文件测试，包含：

- **显式调用**：直接提及 skill 名称
- **隐式调用**：描述场景但不提 skill 名
- **上下文调用**：带额外噪声的真实场景
- **负向控制**：不应触发 skill 的相邻请求

**5. 轻量级确定性评分**
使用 `codex exec --json` 输出 JSONL 结构化事件，编写确定性检查：
...
**6. 基于评分标准的质量检查**
对于风格、规范等难以用规则量化的事项，使用 `--output-schema` 约束 Codex 输出结构化 JSON，进行评分标准（rubric）评估。例如检查：

- Tailwind 配置是否正确
- 组件是否为函数式组件
- 是否使用 Tailwind 工具类而非 CSS 模块
...
- 衡量重要的事，让回归清晰可见
- 从可检查的定义出发，用 `$skill-creator` 快速启动
- 基于行为捕获 JSONL，编写确定性检查
- 规则不足时，用结构化评分标准补充
- 将每次手动修复转化为测试用例
